...
def generate_and_score(model, device, data: DataLoader, n_images, n_recursions, batch_size):
    assert n_recursions > 0, f"n_recursions must be greater than 0! Got {n_recursions}"
    sampled_images = torch.tensor([])
    real_images = torch.tensor([])
    with torch.no_grad():
        for i, (x, _) in tqdm(enumerate(data), total=np.ceil(n_images/batch_size) if n_images else len(data)):
    ...
    fid_score, inception_score, inception_deviation = evaluate_generator(generated_images=normalized_sampled_images, real_images=normalized_real_images, batch_size=200, normalized_images=True, device=device)    

    print(f'FID: {fid_score}, IS: {inception_score}, IS deviation: {inception_deviation}')
    print(f'Sample sizes, sampled: {normalized_sampled_images.shape}, real: {normalized_real_images.shape}')
